# CLAUDE.md — ManagedCode · Mission

Project rules and context for Claude Code (and humans). Read this before working in the repo.
...
- After any intentional visual change: `npm run test:update`, then a clean `npm test` to confirm stability,
  and commit the new baselines. For Linux baselines, use the manual Playwright workflow artifact or run the
  update inside the pinned Playwright container; CI must not commit back to `main`.
